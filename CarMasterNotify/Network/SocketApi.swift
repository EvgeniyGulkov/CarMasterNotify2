...
class SocketClient<T:RawRepresentable> where T.RawValue == String {
    ...
    init(nsp: String = "/") {
        self.manager = SocketManager(socketURL: Constants.CarMasterApi.baseUrl,
                                     config: [.log(false),
                                              .compress,
                                              .extraHeaders(["Authorization" : SecureManager.accessToken])])
    ...
    func emit (event: T, with data:[Any], completion: @escaping (() -> Void)) {
