...
class SocketClient<T:RawRepresentable> where T.RawValue == String {
    ...
    init(nsp: String = "/") {
        self.manager = SocketManager(socketURL: URL(string: "http://192.168.0.104:8000")!, config: [.log(false),.compress, .extraHeaders(["Authorization" : SecureManager.accessToken])])
